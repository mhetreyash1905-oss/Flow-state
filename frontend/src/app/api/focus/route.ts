...
export async function GET(req: Request) {
  try {
    ...
    const result = await getUserFocusSessions(session.user.id, page, limit, status);
...
export async function POST(req: Request) {
  try {
    ...
    const parsedData = createFocusSessionSchema.parse(body);

    const focusSession = await startFocusSession(session.user.id, parsedData);
